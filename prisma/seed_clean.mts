...
async function main() {
  ...
  const superAdminPassword = await hash('AdminMetaForce2026!', 10)
  ...
  const adminPassword = await hash('AdminCenter2026!', 10)
  
  const centerAdmins = await prisma.user.createManyAndReturn({
    data: centers.map((center, index) => ({
      email: `admin${index + 1}@${center.name.toLowerCase().replace(/\s/g, '')}.es`,
      name: faker.person.fullName({ firstName: 'Admin', lastName: `Centro${index + 1}` }),
  ...
  console.log(`Created ${centerAdmins.length} center administrators`)
  ...
  centerAdmins.forEach((admin, index) => {
    console.log(`${index + 1}. Email: ${admin.email} | Password: AdminCenter2026!`)
  })
